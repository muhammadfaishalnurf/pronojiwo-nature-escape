...
const defaultDestinations = [
    {
        ...
        rating: 4.9,
        reviewCount: 120,
        image: "https://i.postimg.cc/3rGp9PfJ/pexels-onesecondbeforesunset-31934675.jpg",
    ...
    },
    {
        ...
        rating: 4.7,
        reviewCount: 85,
        image: "https://i.postimg.cc/MT83Hynz/Wisata-ini-lagi-hits-di-Lumajang.jpg",
    ...
    },
    {
        ...
        rating: 4.8,
        reviewCount: 98,
        image: "https://i.postimg.cc/RZJNVfrG/pexels-joni-agung-438595839-32292604.jpg",
    ...
    },
    {
        ...
        rating: 4.8,
        reviewCount: 74,
        image: "https://i.postimg.cc/cLzwF0WB/Full-Size-Render.avif",
    ...
    },
    {
        ...
        rating: 4.6,
        reviewCount: 52,
        image: "https://i.postimg.cc/zGTHFWdy/6d0be126412a1bd4253ebee9430be30c.jpg",
        description: "Simfoni Alam Kali Kebo: Gagahnya Semeru dalam Pantulan Air yang Tenang. Tempat ideal untuk camping keluarga, berfoto, dan menikmati ketenangan.",
        price: 10000,
        capacity: 300,
        facilities: ["Area camping", "Spot foto estetik", "Warung makan", "Area parkir"],
        coordinates: "8.2111° S, 112.9102° E"
    },
    {
        id: 6,
        name: "Panorama Coban Sriti",
        location: "Pronojiwo, Lumajang",
        category: "Panorama",
        rating: 4.7,
        reviewCount: 40,
        image: "https://i.postimg.cc/T2Wb3Zrb/14094-panorama-coban-sriti-dok-pribadialfaalfnsyh.jpg",
        description: "Potret megah Coban Sriti dengan aliran airnya yang deras berselimut kabut tebal di tengah rimbunnya hutan tropis. Keindahannya kian sempurna dengan latar belakang gagahnya Gunung Sriti di bawah semburat hangatnya cahaya senja.",
        price: 15000,
        capacity: 100,
        facilities: ["Gazebo santai", "Area parkir"],
...
function CustomDatePicker({ value, onChange, isDark }) {
    ...
    const handleDaySelect = (d) => {
        ...
        onChange(`${d.year}-${formattedMonth}-${formattedDay}`);
        setIsOpen(false);
    };
                            ...
                            <button key={i} type="button" onClick={() => handleDaySelect(d)} disabled={d.isPast}
                                className={`aspect-square w-full text-xs font-bold rounded-lg flex flex-col items-center justify-center relative transition-all duration-200 ${d.isPast
                                    ...
                                    : isSelected
                                        ...
                                        : isDark ? 'hover:bg-white/10 text-white/80' : 'hover:bg-stone-100 text-gray-700'
                                    }`}>
                                <span>{d.day}</span>
                                {isToday && !isSelected && (
...
export default function Home() {
    ...
    // ── Set page title ──
    useEffect(() => {
        document.title = "Pronojiwo Nature Escape - Surga Tersembunyi Lumajang";
    }, []);
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                // Fetch destinations dari GET /api/v1/destinations
            ...
            try {
                // Fetch reviews dari GET /api/v1/reviews
    ...
    // ── Animated counter — FIXED: tidak pakai props stats ──
    useEffect(() => {
        if (!visibleStats) return;
        const steps = 60;
        let step = 0;

        // Gunakan data dari API kalau ada, fallback ke default
        const targetWisata  = apiStats?.total_destinasi  || destinations.length || 6;
        const targetReview  = apiStats?.total_review      || 1500;
        const targetRating  = apiStats?.avg_rating        || 4.8;
    ...
    }, [visibleStats, apiStats, destinations.length]);

    // ── Auto testimonial slider — FIXED: dependency array benar ──
    useEffect(() => {
    ...
    // ── Proses destinations dari API atau pakai default ──
    const activeDestinations = destinations.length > 0
        ? destinations.map((d, index) => ({
            ...
            coordinates: (d.nama_wisata || "").toLowerCase().includes("sewu")
                ? "8.2291° S, 112.9157° E"
                : "8.2195° S, 112.9234° E"
        }))
        : defaultDestinations;

    // ── Proses testimonials dari API atau pakai default ──
    ...
    // ── Handle booking submit — kirim ke Laravel API kalau user login ──
    const handleQuickBookingSubmit = async (e) => {
        ...
        if (token) {
            // User sudah login — kirim ke API Laravel
            setBookingLoading(true);
            try {
                const res = await api.post('/tickets', {
                    destination_id: bookingDestId,
                ...
                setCreatedTicket({
                    ...
                    date: bookingDate,
                    quantity: bookingQty,
                    total: ticket.total_price || bookingTotal,
                    pricePerTicket: destination.price
                });
            } catch (err) {
                // Kalau API error, tetap tampilkan tiket lokal
                console.warn('Ticket API error, showing local ticket.');
                setCreatedTicket({
                    code: ticketCode,
                    destination: destination.name,
                    location: destination.location,
            ...
            // User belum login — tampilkan tiket demo
            setCreatedTicket({
                code: ticketCode,
                destination: destination.name,
                location: destination.location,
        ...
        <div className="font-sans text-gray-900 overflow-x-hidden antialiased bg-white selection:bg-emerald-600 selection:text-white">
            ...
            {/* <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${scrolled
                ...
                <div className="max-w-7xl mx-auto px-6 lg:px-8 flex items-center justify-between h-22">
                    ...
                    <div className="hidden lg:flex items-center gap-4">
                        <Link to="/login"
                            className={`px-6 py-2.5 rounded-full font-bold text-sm tracking-wide transition-all duration-300 border-2 ${scrolled
                                ? 'border-emerald-600 text-emerald-700 hover:bg-emerald-600 hover:text-white shadow-md'
                    ...
                    <button className="lg:hidden p-3 rounded-2xl hover:bg-emerald-500/10 transition-colors duration-300"
            ...
            <div className={`lg:hidden fixed top-0 right-0 bottom-0 z-[9999] w-[88vw] max-w-sm flex flex-col transform transition-all duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] ${menuOpen ? 'translate-x-0' : 'translate-x-full'}`}
                ...
                <div className="flex items-center justify-between px-6 pt-8 pb-5">
                    <div className="flex items-center gap-3">
                        <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-emerald-500 to-teal-600 flex items-center justify-center text-white font-black text-sm shadow-xl">WAP</div>
                        <div className="leading-tight">
                    ...
                    <button className="w-10 h-10 rounded-xl border border-white/10 bg-white/5 flex items-center justify-center text-white/70 hover:text-white transition-all"
                        onClick={() => setMenuOpen(false)}>
                ...
                <div className="flex-1 overflow-y-auto px-4 pb-4">
                    <div className="flex flex-col gap-2 mt-4">
                        {navLinks.map((link) => (
                            <a key={link.id} href={`#${link.id}`}
                                className="flex items-center gap-4 px-4 py-4 rounded-2xl border border-white/5 bg-white/[0.03] hover:bg-white/[0.07] text-white font-bold text-sm transition-all"
                                onClick={() => setMenuOpen(false)}>
                                {link.name}
                            </a>
                        ))}
                ...
                <div className="px-5 pt-4 pb-8">
                    <Link to="/login"
                        className="flex items-center justify-center w-full py-3.5 rounded-2xl font-bold text-sm text-white border border-white/15 bg-white/[0.05] hover:bg-white/[0.10] transition-all mb-3"
                        onClick={() => setMenuOpen(false)}>
                        Masuk ke Akun
                    </Link>
                    <Link to="/register"
                        className="flex items-center justify-center w-full py-4 rounded-2xl font-black text-sm text-emerald-950 bg-gradient-to-r from-emerald-400 to-teal-400 shadow-2xl transition-all"
                        onClick={() => setMenuOpen(false)}>
            ...
            <section id="beranda" className="relative min-h-screen flex items-center justify-center px-6 lg:px-8 pt-32 pb-28 overflow-hidden bg-emerald-950">
                ...
                <div className="relative z-10 max-w-7xl w-full mx-auto grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 items-center">
                    <div className="lg:col-span-7 text-left space-y-6">
                        <div className="inline-flex items-center gap-2.5 px-3.5 py-1.5 rounded-full border border-emerald-500/30 bg-emerald-950/45 backdrop-blur-md">
                            <span className="relative flex h-2 w-2">
                                <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-75"></span>
                                <span className="relative inline-flex rounded-full h-2 w-2 bg-emerald-400"></span>
                            </span>
                            <span className="text-emerald-300 text-[10px] font-extrabold tracking-widest uppercase">✦ ECO-PARADISE INDONESIA</span>
                        </div>

                        <h1 className="text-5xl md:text-7xl font-bold leading-[1.2] text-white">
                            Jelajahi Serpihan
                            <span className="block italic font-medium bg-gradient-to-r from-emerald-300 via-teal-200 to-amber-200 text-transparent bg-clip-text mt-4 mb-2">
                                Surga Tersembunyi
                            </span>
                        ...
                        <div className="flex flex-wrap gap-4 pt-4">
                            <a href="#destinasi"
                                className="group px-8 py-4 rounded-full font-bold text-base text-white bg-gradient-to-r from-emerald-600 to-teal-500 hover:from-emerald-500 hover:to-teal-400 shadow-2xl transition-all duration-300 hover:scale-105 flex items-center gap-2.5">
                                Mulai Menjelajah
                            ...
                            <a href="#tiket"
                                className="px-8 py-4 rounded-full font-bold text-base text-white border-2 border-white/30 hover:border-white hover:bg-white/5 transition-all duration-300 flex items-center gap-2.5">
                                Pesan E-Tiket
                            </a>
                        </div>
                            ...
                            {!bookingSuccess ? (
                                <form onSubmit={handleQuickBookingSubmit} className="space-y-5">
                                    ...
                                    <div className="space-y-1.5">
                                        ...
                                        <CustomSelect value={bookingDestId} onChange={setBookingDestId}
                                            options={activeDestinations} placeholder="-- Pilih Destinasi --" isDark={true} />
                                    </div>
                                    <div className="grid grid-cols-2 gap-4">
                                        ...
                                        <div className="space-y-1.5">
                                            ...
                                            <div className="flex items-center bg-emerald-950/50 border border-white/20 rounded-xl px-2 py-1.5">
                                                <button type="button" onClick={() => setBookingQty(Math.max(1, bookingQty - 1))}
                                                    className="w-8 h-8 rounded-lg bg-white/10 text-white font-bold flex items-center justify-center hover:bg-white/20">-</button>
                                                <span className="flex-1 text-center text-white font-black text-sm">{bookingQty}</span>
                                                <button type="button" onClick={() => setBookingQty(bookingQty + 1)}
                                                    className="w-8 h-8 rounded-lg bg-white/10 text-white font-bold flex items-center justify-center hover:bg-white/20">+</button>
                                            </div>
                                        </div>
                                    </div>
                                    {selectedBookingDest && (
                                        <div className="bg-white/5 border border-white/10 rounded-2xl p-4 space-y-2 text-xs">
                                            ...
                                            <div className="border-t border-white/10 pt-2 flex justify-between font-bold text-sm">
                                                <span className="text-amber-300">Total Biaya:</span>
                                                <span className="text-lg text-white font-black">{formatPrice(bookingTotal)}</span>
                                            </div>
                                        </div>
                                    )}
                                    <button type="submit" disabled={bookingLoading}
                                        className="w-full py-4 rounded-xl bg-gradient-to-r from-amber-400 to-yellow-500 hover:from-amber-300 hover:to-yellow-400 text-emerald-950 font-black text-sm uppercase tracking-widest shadow-xl transition-all duration-300 disabled:opacity-60">
                                        {bookingLoading ? "Memproses..." : "Pesan Tiket Sekarang"}
                                    </button>
                                </form>
                            ) : (
                                <div className="space-y-6 text-center py-4">
                                    <div className="w-16 h-16 rounded-full bg-emerald-500/20 border border-emerald-500 flex items-center justify-center mx-auto text-3xl animate-bounce">✅</div>
                                    <div>
                                        <h3 className="text-2xl font-bold text-white mb-1">Pemesanan Sukses!</h3>
                                        <p className="text-emerald-200 text-xs">Tunjukkan kode berikut di loket wisata.</p>
                                    </div>
                                    <div className="print-ticket-card bg-white text-gray-900 rounded-2xl p-5 shadow-2xl border-2 border-amber-300 text-left">
                                        <div className="flex justify-between items-start border-b border-gray-100 pb-3 mb-3">
                                            <div>
                                                <span className="text-[10px] font-bold text-emerald-700 tracking-wider block">WISATA ALAM PRONOJIWO</span>
                                                <h4 className="font-bold text-sm text-gray-950">{createdTicket?.destination}</h4>
                                            </div>
                    ...
                    <div className="bg-white/5 backdrop-blur-md border border-white/10 rounded-3xl p-8 md:p-12 shadow-2xl">
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 text-center divide-y md:divide-y-0 md:divide-x divide-white/10">
                            <div className="py-6 md:py-0 md:px-6 space-y-2">
                                <div className="text-5xl font-black text-amber-300">{counts.wisata}+</div>
                                <div className="text-lg font-bold text-white italic">Total Destinasi</div>
                                <p className="text-white/60 text-xs max-w-[200px] mx-auto">Kawasan wisata alam terintegrasi dan teregistrasi.</p>
                            </div>
                            <div className="py-6 md:py-0 md:px-6 space-y-2">
                                <div className="text-5xl font-black text-white">{counts.pengunjung.toLocaleString()}+</div>
                                <div className="text-lg font-bold text-white italic">Review Wisatawan</div>
                                <p className="text-white/60 text-xs max-w-[200px] mx-auto">Ulasan nyata bintang 4 keatas oleh pelancong.</p>
                            </div>
                            <div className="py-6 md:py-0 md:px-6 space-y-2">
                                <div className="text-5xl font-black text-emerald-400 flex items-center justify-center gap-1">
                                    {counts.rating} <span className="text-amber-400 text-3xl">★</span>
                                </div>
                                <div className="text-lg font-bold text-white italic">Rating Kepuasan</div>
                ...
                <div className="max-w-7xl mx-auto">
                    <div className="text-center max-w-3xl mx-auto mb-16 space-y-4">
                        ...
                        <h2 className="text-4xl md:text-5xl font-bold tracking-tight text-gray-950">
                            Jelajahi Katalog <span className="italic font-medium text-emerald-700">Destinasi Favorit</span>
                        </h2>
                        ...
                        {categories.map(cat => (
                            <button key={cat} onClick={() => setSelectedCategory(cat)}
                                className={`px-6 py-2.5 rounded-full font-bold text-xs tracking-wide shadow-sm transition-all duration-300 border ${selectedCategory === cat
                                    ...
                                    : 'bg-stone-50 text-gray-600 border-gray-100 hover:bg-stone-100'}`}>
                    ...
                    {loadingData ? (
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                            {[...Array(6)].map((_, i) => (
                                <div key={i} className="bg-stone-100 rounded-3xl h-96 animate-pulse"></div>
                            ))}
                        </div>
                    ) : (
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                            {filteredDestinations.map((dest) => (
                                <div key={dest.id}
                                    className="group bg-stone-50 rounded-3xl overflow-hidden shadow-md hover:shadow-2xl border border-gray-100 transition-all duration-500 flex flex-col">
                                    <div className="relative h-64 overflow-hidden">
                                        ...
                                        <span className="absolute top-4 left-4 bg-emerald-900/90 text-white text-[10px] font-extrabold px-3 py-1.5 rounded-full uppercase tracking-wider">
                                            {dest.category}
                                        </span>
                                        <span className="absolute top-4 right-4 bg-gray-950/60 text-white text-xs font-bold px-3 py-1.5 rounded-full flex items-center gap-1">
                                            <span className="text-amber-400">★</span> {dest.rating}
                                        </span>
                                        <span className="absolute bottom-4 left-4 text-white font-black text-lg bg-emerald-700/80 px-4 py-1.5 rounded-xl">
                                    ...
                                    <div className="p-6 flex-1 flex flex-col justify-between">
                                        <div className="space-y-3">
                                            <div className="flex items-center gap-1.5 text-xs text-gray-400 font-semibold">
                                                <svg className="w-3.5 h-3.5 text-emerald-600 flex-shrink-0" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
                                                    <path strokeLinecap="round" strokeLinejoin="round" d="M15 10.5a3 3 0 11-6 0 3 3 0 016 0z M19.5 10.5c0 7.142-7.5 11.25-7.5 11.25S4.5 17.642 4.5 10.5a7.5 7.5 0 1115 0z"></path>
                                                </svg>
                                                <span>{dest.location}</span>
                                            </div>
                                            <h3 className="text-xl font-bold text-gray-950 group-hover:text-emerald-700 transition-colors line-clamp-1">
                                                {dest.name}
                                            </h3>
                                        ...
                                        <div className="border-t border-gray-100 pt-5 mt-5 flex items-center justify-between">
                                            ...
                                            <button onClick={() => setSelectedDestDetails(dest)}
            ...
            <section className="bg-stone-50 py-24 px-6 lg:px-8 relative overflow-hidden">
                <div className="max-w-7xl mx-auto">
                    <div className="text-center max-w-3xl mx-auto mb-16 space-y-4">
                        ...
                        <h2 className="text-4xl md:text-5xl font-bold tracking-tight text-gray-950">
                            Kenapa Memilih <span className="italic font-medium text-emerald-700">Layanan Kami?</span>
                        </h2>
                        <div className="w-16 h-1 bg-gradient-to-r from-emerald-600 to-teal-500 mx-auto rounded-full"></div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                        {features.map((item, index) => (
                            <div key={item.title}
                                className="group bg-white/80 border border-gray-100/60 rounded-[32px] p-8 text-center shadow-sm hover:shadow-xl hover:-translate-y-2 transition-all duration-500 relative overflow-hidden">
                                <div className="mb-6 flex justify-center">
                                    <div className={`w-16 h-16 bg-white rounded-2xl shadow-sm group-hover:bg-gradient-to-br group-hover:${item.gradient} group-hover:scale-105 transition-all duration-500 flex items-center justify-center text-3xl`}>
                                        {item.icon}
                                    </div>
                                </div>
                                <h3 className="font-extrabold text-xl text-gray-900 mb-3.5 group-hover:text-emerald-700 transition-colors">
                                    {item.title}
                                </h3>
                ...
                <div className="relative max-w-4xl mx-auto z-10">
                    <div className="text-center space-y-4 mb-16">
                        ...
                        <h2 className="text-4xl md:text-5xl font-bold tracking-tight text-white">
                            Kisah Seru <span className="italic font-medium bg-gradient-to-r from-emerald-300 to-amber-200 text-transparent bg-clip-text">Mereka di Pronojiwo</span>
                        </h2>
                    </div>
                                ...
                                <div className="bg-white/5 border border-white/15 backdrop-blur-xl rounded-3xl p-8 md:p-10 shadow-2xl">
                                    ...
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            ...
                                            <div className="text-emerald-300 text-xs font-semibold">🗺️ {test.location}</div>
                        ...
                        {activeTestimonials.map((_, index) => (
                            <button key={index} onClick={() => setActiveTestimonial(index)}
                                className={`h-2.5 rounded-full transition-all duration-500 ${index === activeTestimonial
                                    ? "w-10 bg-gradient-to-r from-emerald-400 to-teal-300"
            ...
            <section id="tiket" className="relative py-24 bg-stone-50 px-6 lg:px-8 border-b border-gray-100">
                <div className="max-w-4xl mx-auto">
                    <div className="text-center space-y-4 mb-16">
                        ...
                        <h2 className="text-4xl md:text-5xl font-bold tracking-tight text-gray-950">
                            Pesan E-Tiket <span className="italic font-medium text-emerald-700">Digital Anda</span>
                        </h2>
                        <div className="w-16 h-1 bg-gradient-to-r from-emerald-600 to-teal-500 mx-auto rounded-full"></div>
                        <p className="text-gray-500 text-sm max-w-xl mx-auto">
                            Kalkulasikan langsung biaya liburan Anda. Dapatkan kode konfirmasi instan tanpa antre di gerbang masuk.
                        </p>
                    </div>
                            ...
                            <form onSubmit={handleQuickBookingSubmit} className="space-y-6">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div className="space-y-2">
                                        ...
                                        <CustomSelect value={bookingDestId} onChange={setBookingDestId}
                                            options={activeDestinations} placeholder="-- Pilih Destinasi Favorit --" isDark={false} />
                                    </div>
                                    <div className="space-y-2">
                                        <label className="text-gray-700 font-extrabold text-xs uppercase tracking-wider block">Tanggal Kunjungan</label>
                                        <CustomDatePicker value={bookingDate} onChange={setBookingDate} isDark={false} />
                                    </div>
                                </div>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-center">
                                    <div className="space-y-2">
                                        ...
                                        <div className="flex items-center bg-stone-50 border border-gray-200 rounded-xl px-3 py-2 max-w-[200px]">
                                            <button type="button" onClick={() => setBookingQty(Math.max(1, bookingQty - 1))}
                                                className="w-8 h-8 rounded-lg bg-gray-200 text-gray-700 font-bold flex items-center justify-center hover:bg-gray-300">-</button>
                                    ...
                                    {selectedBookingDest && (
                                        <div className="bg-emerald-50 border border-emerald-100 rounded-xl p-3.5 text-xs text-emerald-800">
                                            💡 5% dari setiap tiket berkontribusi ke program konservasi Pronojiwo.
                                        </div>
                                    )}
                                </div>
                                ...
                                {selectedBookingDest ? (
                                    <div className="bg-stone-50 border border-gray-200 rounded-2xl p-6 space-y-3 text-sm">
                                        ...
                                        <div className="flex justify-between text-gray-600">
                                            <span>Tiket ({selectedBookingDest.name}):</span>
                                            <span className="font-semibold">{formatPrice(selectedBookingDest.price)} x {bookingQty}</span>
                                        </div>
                                        <div className="flex justify-between text-gray-600">
                                            <span>Biaya Layanan (5%):</span>
                                            <span className="font-semibold">{formatPrice(bookingTax)}</span>
                                        </div>
                                        <div className="border-t border-gray-200 pt-3 flex justify-between font-black text-base text-emerald-800">
                                            <span>Total:</span>
                                            <span className="text-xl text-gray-950">{formatPrice(bookingTotal)}</span>
                                        </div>
                                    </div>
                                ) : (
                                    <div className="text-center py-6 border-2 border-dashed border-gray-200 rounded-2xl text-gray-400 text-sm">
                                        Isi form di atas untuk melihat kalkulasi biaya tiket.
                                    </div>
                                )}

                                <button type="submit" disabled={bookingLoading}
                                    className="w-full py-4 rounded-xl bg-gradient-to-r from-emerald-700 to-teal-600 hover:from-emerald-600 hover:to-teal-500 text-white font-bold text-sm uppercase tracking-widest shadow-xl transition-all duration-300 disabled:opacity-60">
                                    {bookingLoading ? "Memproses..." : "Pesan E-Tiket Sekarang"}
                                </button>
                            </form>
                        ) : (
                            <div className="text-center py-8 space-y-6 max-w-md mx-auto">
                                <div className="w-20 h-20 bg-emerald-100 rounded-full flex items-center justify-center mx-auto text-4xl text-emerald-600">✓</div>
                                <div>
                                    <h3 className="text-3xl font-black text-gray-950">Transaksi Berhasil!</h3>
                                    <p className="text-gray-500 text-sm mt-2">Simpan kode tiket ini untuk ditukarkan di gerbang masuk wisata.</p>
                                </div>
                                <div className="print-ticket-card bg-white border-2 border-emerald-600 rounded-3xl p-6 shadow-xl text-left">
                                    <div className="w-full h-3 bg-gradient-to-r from-emerald-600 to-teal-500 rounded-t-xl -mt-6 -mx-6 mb-4 px-6"></div>
                                    <div className="flex justify-between items-start border-b border-gray-100 pb-4 mb-4">
                                        <div>
                                            <h4 className="font-extrabold text-xs text-emerald-800 tracking-wider">E-PRONOJIWO NATURE ESCAPE</h4>
                                            <h5 className="font-black text-base text-gray-950 mt-0.5">{createdTicket?.destination}</h5>
                                        </div>
                                        <div className="text-right">
                                    ...
                                    <div className="grid grid-cols-2 gap-4 text-xs">
                                        ...
                                        <div>
                                            <span className="text-gray-400 block text-[10px] uppercase font-bold">Jumlah</span>
                                        ...
                                        <div>
                                            ...
                                            <span className="inline-block px-2 py-0.5 rounded-full text-[9px] font-extrabold bg-emerald-50 text-emerald-700 uppercase border border-emerald-100">Aktif</span>
                                        </div>
                                    </div>
                                </div>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
                        <div className="lg:col-span-5 space-y-6">
                            ...
                            <p className="text-gray-500 text-sm leading-relaxed">
                                Punya kendala rute, butuh akomodasi homestay, atau ingin merancang trip rombongan? Tim kami siap melayani 24/7.
                            </p>
                            <div className="space-y-4 pt-4">
                                {[
                                    { icon: "📍", title: "Kantor Informasi Wisata", text: "Kecamatan Pronojiwo, Lumajang, Jawa Timur" },
                                    ...
                                    <div key={c.text} className="flex gap-4 p-4 rounded-2xl bg-stone-50 border border-gray-100 hover:border-emerald-100 transition-colors">
                                        <div className="w-10 h-10 rounded-xl bg-emerald-50 flex items-center justify-center text-lg flex-shrink-0">{c.icon}</div>
                                        <div>
                        ...
                        <div className="lg:col-span-7 bg-stone-50 border border-gray-200/50 rounded-3xl p-6 md:p-8 shadow-xl">
                            ...
                            <form onSubmit={(e) => { e.preventDefault(); alert("Pesan Anda telah terkirim! Tim kami akan membalas dalam 1x24 jam."); e.target.reset(); }}
                                className="space-y-4">
                                <div className="grid grid-cols-2 gap-4">
                                    <div className="space-y-1">
                                        <label className="text-xs font-bold text-gray-500">Nama Lengkap</label>
                                        <input type="text" placeholder="cth: Ahmad Dani" required
                                            className="w-full bg-white border border-gray-200 rounded-xl px-4 py-3 text-xs text-gray-900 focus:outline-none focus:border-emerald-600 transition-all" />
                                    </div>
                                    <div className="space-y-1">
                                        <label className="text-xs font-bold text-gray-500">Alamat Surel</label>
                                        <input type="email" placeholder="cth: dani@email.com" required
                                            className="w-full bg-white border border-gray-200 rounded-xl px-4 py-3 text-xs text-gray-900 focus:outline-none focus:border-emerald-600 transition-all" />
                                    </div>
                                </div>
                                <div className="space-y-1">
                                    <label className="text-xs font-bold text-gray-500">Subjek Pertanyaan</label>
                                    <input type="text" placeholder="cth: Sewa Guide lokal / Reservasi Homestay" required
                                        className="w-full bg-white border border-gray-200 rounded-xl px-4 py-3 text-xs text-gray-900 focus:outline-none focus:border-emerald-600 transition-all" />
                                </div>
                                <div className="space-y-1">
                                    <label className="text-xs font-bold text-gray-500">Isi Pesan</label>
                                    <textarea rows="4" placeholder="Tuliskan pertanyaan Anda..." required
                                        className="w-full bg-white border border-gray-200 rounded-xl px-4 py-3 text-xs text-gray-900 focus:outline-none focus:border-emerald-600 transition-all resize-none"></textarea>
                                </div>
                                <button type="submit"
                                    className="w-full py-3.5 rounded-xl bg-emerald-800 hover:bg-emerald-700 text-white font-bold text-xs uppercase tracking-wider shadow-lg transition-all duration-300">
                ...
                <div className="max-w-7xl mx-auto">
                    <div className="grid grid-cols-1 md:grid-cols-12 gap-12 pb-16 border-b border-white/10">
                        <div className="md:col-span-5 space-y-6">
                            <div className="flex items-center gap-3">
                                <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-emerald-600 to-teal-500 flex items-center justify-center text-white font-black text-sm shadow-xl">WAP</div>
                                <div className="leading-tight">
                            ...
                            <p className="text-sm text-white/60 leading-relaxed max-w-sm font-light">
                                Pronojiwo Nature Escape adalah media resmi reservasi & pariwisata terpadu kawasan Kecamatan Pronojiwo, Lumajang.
                            </p>
                        </div>

                        <div className="md:col-span-3 space-y-5">
                            <h4 className="text-white font-bold text-sm uppercase tracking-widest border-l-2 border-emerald-500 pl-3">Navigasi</h4>
                            <div className="flex flex-col gap-3.5 text-sm font-semibold">
                                {navLinks.map((link) => (
                                    <a key={link.id} href={`#${link.id}`} className="hover:text-emerald-400 transition-colors w-fit">→ {link.name}</a>
                                ))}
                            </div>
                        </div>
                        ...
                        <div className="md:col-span-4 space-y-5">
                            ...
                            <p className="text-xs text-white/50 leading-relaxed">Dapatkan info promo musiman dan update wisata Pronojiwo gratis.</p>
                            <form onSubmit={(e) => { e.preventDefault(); alert("Terima kasih! Anda berlangganan info Pronojiwo."); e.target.reset(); }}
                                className="flex flex-col sm:flex-row gap-2">
                                <input type="email" placeholder="Alamat surel Anda" required
                                ...
                                <button type="submit"
                                    className="px-5 py-3 rounded-xl bg-emerald-600 hover:bg-emerald-500 font-bold text-xs text-white transition-colors">
                                    Gabung
                                </button>
                            </form>
                        </div>
            ...
            {selectedDestDetails && (
                <div className="fixed inset-0 z-50 overflow-y-auto bg-gray-950/80 backdrop-blur-md flex items-center justify-center p-4 md:p-6">
                    <div className="bg-white border border-gray-100 rounded-3xl max-w-2xl w-full overflow-hidden shadow-2xl relative">
                        <button onClick={() => setSelectedDestDetails(null)}
                            className="absolute top-4 right-4 z-10 w-9 h-9 rounded-full bg-gray-950/40 hover:bg-gray-950/80 text-white flex items-center justify-center transition-colors">
                            ✕
                        </button>
                        <div className="h-64 relative">
                            <img src={selectedDestDetails.image} alt={selectedDestDetails.name} className="w-full h-full object-cover" />
                            ...
                            <div className="absolute bottom-4 left-6 space-y-1">
                                ...
                                <h3 className="text-2xl md:text-3xl font-bold text-white">{selectedDestDetails.name}</h3>
                            </div>
                        </div>
                        ...
                        <div className="p-6 md:p-8 space-y-6 max-h-[50vh] overflow-y-auto">
                            <div className="grid grid-cols-3 gap-2 border-b border-gray-100 pb-4 text-center">
                                <div>
                                    <span className="text-[10px] text-gray-400 font-bold uppercase block">Harga Masuk</span>
                                    <span className="font-black text-sm text-emerald-800">{formatPrice(selectedDestDetails.price)}</span>
                                </div>
                                <div>
                                    <span className="text-[10px] text-gray-400 font-bold uppercase block">Koordinat</span>
                                    <span className="font-bold text-gray-600 text-xs">{selectedDestDetails.coordinates}</span>
                                </div>
                                <div>
                            ...
                            <div className="space-y-2">
                                <h4 className="text-xs font-extrabold uppercase text-gray-400 tracking-wider">Deskripsi</h4>
                                <p className="text-gray-600 text-sm leading-relaxed">{selectedDestDetails.description}</p>
                            </div>
                            <div className="space-y-3">
                                <h4 className="text-xs font-extrabold uppercase text-gray-400 tracking-wider">Fasilitas</h4>
                                <div className="flex flex-wrap gap-2">
                                    {selectedDestDetails.facilities.map((fac, idx) => (
                                        <span key={idx} className="px-3 py-1.5 rounded-xl bg-stone-50 border border-gray-100 text-xs font-semibold text-gray-600">
                                            🏕️ {fac}
                                        </span>
                                    ))}
                                </div>
                        ...
                        <div className="bg-stone-50 border-t border-gray-100 p-6 flex justify-end gap-3">
                            ...
                            <button onClick={() => handleDirectBook(selectedDestDetails)}
                                className="px-6 py-2.5 rounded-full bg-emerald-800 hover:bg-emerald-700 text-white font-bold text-xs uppercase tracking-wider shadow-lg transition-all">
            ...
            <style>{`
                ...
                @media print {
                    @page { size: 600px 380px; margin: 0; }
                    html, body { height: 380px !important; overflow: hidden !important; margin: 0 !important; padding: 0 !important; background: #fff !important; }
                    body * { visibility: hidden !important; }
                    .print-ticket-card, .print-ticket-card * { visibility: visible !important; }
                    .print-ticket-card { position: fixed !important; left: 0 !important; top: 0 !important; width: 600px !important; height: 380px !important; margin: 0 !important; padding: 24px !important; border: none !important; box-shadow: none !important; background: white !important; z-index: 9999999 !important; }
                }
            `}</style>
        </div>
    );
