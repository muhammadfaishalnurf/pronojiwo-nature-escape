...
export default function SADashboard() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                // Set SEKALI dari API — tidak ada default yang bisa bocor masuk
    ...
    const statCards = [
        { label: "Total Pengguna",   value: stats.total_users      || 0, icon: "👥", color: "from-violet-500 to-purple-600", isText: false },
        { label: "Total Destinasi",  value: stats.total_destinasi  || 0, icon: "🏔️", color: "from-emerald-500 to-teal-600",  isText: false },
        { label: "Total Tiket",      value: stats.total_tiket      || 0, icon: "🎫", color: "from-amber-500 to-orange-500",  isText: false },
        { label: "Total Pendapatan", value: formatRp(stats.total_pendapatan), icon: "💰", color: "from-blue-500 to-cyan-500", isText: true },
    ];
        ...
        <AdminLayout title="Super Admin Dashboard">
            {loading || chartData === null ? (
                ...
                <>
                    ...
                    <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                        {statCards.map(c => (
                            <div key={c.label} className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
                                <div className={`w-12 h-12 rounded-xl bg-gradient-to-br ${c.color} flex items-center justify-center text-2xl shadow-md mb-4`}>
                                    {c.icon}
                                </div>
                                <p className={`font-black text-gray-900 leading-tight mb-1 ${c.isText ? "text-sm" : "text-2xl"}`}>
                                    {c.value}
                                </p>
                    ...
                    {/* Breakdown tiket status */}
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8">
                        {[
                            { label: "Aktif (confirmed)", value: stats.tiket_confirmed || 0, color: "bg-emerald-50 text-emerald-700 border-emerald-100" },
                            { label: "Terpakai (used)",   value: stats.tiket_used      || 0, color: "bg-gray-50 text-gray-500 border-gray-100" },
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
                        ...
                        <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
                            ...
                            <ResponsiveContainer width="100%" height={180}>
                                <PieChart>
                                    <Pie data={roleData} cx="50%" cy="50%" outerRadius={70}
                                        dataKey="value"
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                        {[
                            { label: "Kelola Pengguna & Role", desc: "Tambah, edit, dan atur role pengguna", icon: "👥", link: "/super-admin/pengguna" },
                            { label: "Kelola Destinasi",       desc: "CRUD data wisata Pronojiwo",          icon: "🏔️", link: "/admin/destinasi" },
                            { label: "Pengaturan Sistem",      desc: "Konfigurasi nama app, kontak, dsb",   icon: "⚙️", link: "/super-admin/pengaturan" },
                        ].map(q => (
                            <a key={q.label} href={q.link}
                                ...
                                <div className="w-12 h-12 rounded-xl bg-gray-50 group-hover:bg-emerald-50 flex items-center justify-center text-2xl transition-colors flex-shrink-0">
                                    {q.icon}
                                </div>
                                <div>
