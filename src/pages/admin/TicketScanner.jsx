...
export default function AdminTicketScanner() {
    ...
    // Load html5-qrcode saat mode kamera dipilih
    useEffect(() => {
        if (mode !== "camera") {
            stopCamera();
            return;
    ...
    const startCamera = async () => {
        try {
            ...
            setMessage({ type: "error", text: "Kamera tidak bisa diakses. Gunakan mode manual." });
            setMode("manual");
        }
    };
    ...
    const handleScan = async (code) => {
        ...
        try {
            ...
            setScanResult(data || { valid: false, message: "Tiket tidak ditemukan." });
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleConfirmEntry = async () => {
        ...
        try {
            ...
            setScanResult({ ...scanResult, ticket: res.data.ticket, valid: false });
        } catch (err) {
        ...
        <AdminLayout>
            <div className="max-w-xl mx-auto px-4 py-8">
                ...
                <div className="flex gap-2 bg-gray-100 p-1 rounded-xl mb-6">
                    {[
                        { key: "manual", label: "✏️ Input Manual", desc: "Ketik / scan QR reader" },
                        { key: "camera", label: "📷 Kamera",       desc: "Scan pakai kamera HP" },
                    ].map(m => (
                ...
                {mode === "manual" && (
                    <form onSubmit={handleManualSubmit} className="mb-6">
                        ...
                        <p className="text-xs text-gray-400 mt-2 text-center">
                            Jika menggunakan QR reader fisik, arahkan ke QR lalu tekan scan — kode otomatis terisi
                        </p>
                    </form>
                )}
                    ...
                    <div className={`rounded-2xl border-2 overflow-hidden ${scanResult.valid ? "border-emerald-400" : ticket?.status === "used" ? "border-gray-300" : "border-red-300"}`}>
                            ...
                            <div className="bg-white p-5 space-y-4">
                                ...
                                <div className="flex gap-3 items-start">
                                    {ticket.destination?.foto && (
                                        <img src={ticket.destination.foto} alt="" className="w-16 h-16 rounded-xl object-cover flex-shrink-0"/>
                                    )}
                                    <div>
                                        ...
                                        <span className={`inline-block mt-1 text-xs font-bold px-2 py-0.5 rounded-full border ${statusColor(ticket.status)}`}>
                                            {ticket.status === "confirmed" ? "Aktif" : ticket.status === "used" ? "Sudah Digunakan" : ticket.status}
                                        </span>
                                    </div>
                                </div>
                                    ...
                                    {ticket.status === "used" && ticket.used_at && (
                                        <div className="col-span-2 bg-red-50 rounded-lg p-2 text-center">
                                            <p className="text-xs text-red-600 font-bold">
                                                Tiket digunakan pada: {new Date(ticket.used_at).toLocaleString("id-ID")}
                                            </p>
                                        </div>
                                    )}
                                </div>
                        ...
                        {!ticket && (
                            <div className="bg-white p-5 text-center">
                                ...
                                <button onClick={reset}
                                    className="px-6 py-2.5 rounded-xl bg-gray-100 text-gray-700 font-bold text-sm hover:bg-gray-200 transition-colors">
                                    Coba Lagi
                                </button>
                            </div>
                        )}
                    ...
                    <div className="mt-8 bg-white rounded-2xl border border-gray-100 p-5">
                        ...
                        <div className="space-y-3">
                            {[
                                ...
                                { icon: "🚪", step: "Klik Konfirmasi Masuk jika tiket valid — tiket langsung tidak bisa dipakai lagi" },
                            ].map((g, i) => (
